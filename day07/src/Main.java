...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < dependenciesStringVec.size(); i++) {
			Depend before = getDependByChar(dependenciesStringVec.get(i)[0]
					.toCharArray()[0]);
			Depend after = getDependByChar(dependenciesStringVec.get(i)[1]
					.toCharArray()[0]);
	...
	public static void process() {
		...
		while (isOpen()) {
			for (int i = 0; i < dependencies.size(); i++) {
				if (dependencies.get(i).before.size() == 0
						&& dependencies.get(i).done == false) {
					dependencies.get(i).done = true;
					order.add(dependencies.get(i).id);
					for (int j = 0; j < dependencies.size(); j++) {
						dependencies.get(j).before
								.remove(getDependByChar(dependencies.get(i).id));
					}
				}
			}
		}
